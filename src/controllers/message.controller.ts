...
export class MessageController {
  ...
  createMessage = async (req: Request, res: Response): Promise<void> => {
    const conversation = await messageService.createMessage(req.params.conversationId as string, req.body as CreateMessageInput, req.file);
    new SuccessResponse({
      statusCode: StatusCodes.OK,
      reasonPhrase: ReasonPhrases.OK,
      data: conversation
  ...
  getConversationById = async (req: Request, res: Response): Promise<void> => {
    const conversation = await messageService.getConversationById(req.params.conversationId as string);
    new SuccessResponse({
      statusCode: StatusCodes.OK,
      reasonPhrase: ReasonPhrases.OK,
      data: conversation
  ...
  readConversationMessages = async (req: Request, res: Response): Promise<void> => {
    const result = await messageService.readConversationMessages(req.currentUser?.sub, req.params.conversationId as string);
    new SuccessResponse({
      statusCode: StatusCodes.OK,
      reasonPhrase: ReasonPhrases.OK,
      data: result
  ...
  getMessagesInConversation = async (req: Request, res: Response): Promise<void> => {
    ...
    const messages: IMessageDocument[] = await messageService.getMessagesInConversation(
      req.currentUser?.sub, conversationId as string, query
    );
    new SuccessResponse({
      statusCode: StatusCodes.OK,
      reasonPhrase: ReasonPhrases.OK,
